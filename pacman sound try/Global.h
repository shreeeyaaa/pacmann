...
constexpr unsigned char GHOST_ESCAPE_SPEED = 4;//4 frames for one unit movemnet
...
constexpr unsigned char GHOST_SPEED = 1;//moves one unit per frame
...
constexpr unsigned char PACMAN_SPEED = 2;//2 units per frame
...
struct Position
{
	//short uses less memory than int so for memory usage optimization
	short x;
	short y;
