...
void Ghost::draw(bool i_flash, sf::RenderWindow& i_window)
{
    ...
    // to control the progression of animation frames, ensuring that it loops back to 0 after reaching the end of the animation sequence.>-
...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	walls[3] = map_collision(0, use_door, position.x, speed + position.y, i_map);//down
